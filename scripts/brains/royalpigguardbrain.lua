...
local function checknotangry(inst)
    local player = GetClosestInstWithTag("player", inst, 4)
	return not inst:HasTag("angry_at_player") or player == nil
end
...
local function shouldPanic(inst)
    ...
    if inst.components.combat.target then
        ...
        if threat ~= nil then
            local dist = distsq(inst:GetPosition(),threat:GetPosition())
...
function RoyalPigGuardBrain:OnStart()
    ...
    local night = WhileNode(function() return TheWorld.state.isday end, "IsNight",
        PriorityNode{
