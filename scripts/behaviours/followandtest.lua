FollowAndTest = Class(BehaviourNode, function(self, inst, target, min_dist, target_dist, max_dist, canrun, alwayseval, inlimbo_invalid, testfn)
    BehaviourNode._ctor(self, "FollowAndTest")
    self.inst = inst
    self.target = target
...
function FollowAndTest:GetTarget()
    local target = FunctionOrValue(self.target, self.inst)

    local cantarget = true
    if self.testfn then
        cantarget = self.testfn(self.inst, target)
    end

    return target ~= nil and target:IsValid() and cantarget and target or nil
end
...
function FollowAndTest:AreDifferentPlatforms(inst, target)
...
function FollowAndTest:Visit()
    ...
    if self.status == RUNNING then
        if self.currenttarget == nil
            ...
            or (self.testfn and not self.testfn(self.inst, self.currenttarget))
        then
            self.status = FAILED
