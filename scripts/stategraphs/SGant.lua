...
local states=
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            local player = GetClosestInstWithTag("player", inst, TUNING.WEEVOLE_SHARE_TARGET_RANGE)
    ...
    State{
        ...
        onenter = function(inst)
            ...
            local antman_warriro_egg = SpawnPrefab("antman_warrior_egg")
            local pos = inst.Transform:GetWorldPosition()
            antman_warriro_egg.Transform:SetPosition(pos)
            antman_warriro_egg.eggify(antman_warriro_egg)
            -- re-register us with the childspawner and interior
            -- ReplaceEntity(inst, egg)

	        antman_warriro_egg:AddTag("aporkalypse_cleanup")
        end,
