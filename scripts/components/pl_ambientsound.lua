...
local AMBIENT_SOUNDS =
{
    ...
    --ATC new
    [WORLD_TILES.SALTBEACH] =          {sound = "turnoftides/together_amb/moon_island/fall",          temperate = "turnoftides/together_amb/moon_island/fall",          lush = "turnoftides/together_amb/moon_island/fall",       humid = "turnoftides/together_amb/moon_island/fall",       aporkalypse = "turnoftides/together_amb/moon_island/fall"},
    [WORLD_TILES.SALTLAKE] =           {sound = "turnoftides/together_amb/ocean/shallow",             temperate = "turnoftides/together_amb/ocean/shallow",             lush = "turnoftides/together_amb/ocean/shallow",          humid = "turnoftides/together_amb/ocean/shallow",          aporkalypse = "turnoftides/together_amb/ocean/shallow"},
...
function self:OnUpdate(dt)
    ...
    elseif _lastplayerpos == nil or player:GetDistanceSqToPoint(_lastplayerpos:Get()) >= 16 then
        ...
        for x1 = -HALF_TILES, HALF_TILES do
            for z1 = -HALF_TILES, HALF_TILES do
                local test_x = x + x1 * 4
                local test_z = z + z1 * 4
                local tile = _map:GetTileAtPoint(test_x, 0, test_z)
                local is_interior = TheWorld.components.interiorspawner:IsInInteriorRegion(test_x, test_z)
                if TileGroupManager:IsImpassableTile(tile) and not is_interior then
    ...
    elseif _lightattenuation and player.LightWatcher ~= nil then
        ...
        local lowlight = 0.2
        local lowvolume = 0.5
        ambientvolume = (lightval > highlight and lowvolume) or (lightval < lowlight and 1)
            or easing.outCubic(lightval - lowlight, 1, lowvolume - 1, highlight - lowlight)
    elseif ambientvolume < 1 then
        ambientvolume = math.min(ambientvolume + dt * 0.05, 1)
    end
    ...
    if player ~= nil and TheWorld.net ~= nil and TheWorld.net.components.plateauwind:GetWindSpeed() > 0 then  -- 在世界刚刚初始化的时候，TheWorld.net可能为nil
