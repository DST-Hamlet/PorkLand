    ...
    if enable then
        if not inst.fullbloom then
            ...
            if not inst.pollentask then
                inst.pollentask = inst:DoPeriodicTask(0.7, PollenTick)
            end
            if not inst.planttask then
                inst.planttask = inst:DoPeriodicTask(0.25, PlantTick)
            end
        end
...
local function SetStatsLevel(inst, stage)
    ...
    local mult = Remap(stage, 0, 3, 1, 1.2)
...
local function OnNewSGState(inst)
    if not inst.sg:HasStateTag("nomorph") then
        inst:SetBloomStage(inst.bloomstage)
    end
end
...
local function StartBloomFn(inst, instant)
    if not instant then
        inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_BLOOMING"))
    end
end
...
local function OnSeasonChange(inst, season)
    if season == SEASONS.LUSH and not inst:HasTag("playerghost") and not inst.components.bloomable:IsBlooming() then
        inst.components.bloomable:StartBloomTask()
    elseif season ~= SEASONS.LUSH and inst.components.bloomable:IsBlooming() then
        inst.components.bloomable:StartUnbloomTask()
    end
end

local function OnSeasonTick(inst, data)
    if inst.components.bloomable.blooming then
        local percent = data.progress
        if percent < 1/6 then
...
local function OnBecameGhost(inst)
    inst.components.bloomable:StartUnbloomTask(0)
...
local function master_postinit(inst)
    ...
    inst.components.bloomable:SetCanBloom(function() return true end)
    ...
    inst:ListenForEvent("seasontick", function(src, data)
        OnSeasonTick(inst, data)
    end, TheWorld)
