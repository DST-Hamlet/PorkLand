...
function AreaAware:GetDebugString()
    ...
    if node then
        local s = string.format("%s: %s [%d]",tostring(TheWorld.topology.ids[self.current_area]),
            table.reverselookup(NODE_TYPE, node.type) or "N/A", self.current_area) -- added default val for node type TODO check why it's missing
        if node.tags then
